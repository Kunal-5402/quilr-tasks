#!/usr/bin/env bash
# Task 4: failover on 429, failover on timeout, and the token budget.
set -euo pipefail
cd "$(dirname "$0")/.."

echo "=== Task 4: rate limiter and model fallback ==="
rm -f var/router.sqlite3*
uv run python -m fde.task4_router --primary   >/dev/null 2>&1 & P=$!
uv run python -m fde.task4_router --secondary >/dev/null 2>&1 & S=$!
uv run python -m fde.task4_router             >/dev/null 2>&1 & G=$!
...
ask() {  # label max_tokens
  printf '  %-26s -> ' "$1"
  curl -s -o /tmp/fde_body -w '%{http_code}' -X POST http://127.0.0.1:8004/v1/completions \
    -H 'Authorization: Bearer tenant-a-key' -H 'Content-Type: application/json' \
    -D /tmp/fde_head -d "{\"prompt\":\"hello gateway\",\"max_tokens\":$2"'}'
  printf ' provider=%s ' "$(grep -i '^x-fde-provider' /tmp/fde_head | tr -d '\r' | cut -d' ' -f2)"
  head -c 150 /tmp/fde_body; echo
}
