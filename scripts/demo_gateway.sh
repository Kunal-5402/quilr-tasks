#!/usr/bin/env bash
# the gateway denies an admin_ tool to a viewer before it calls downstream.
set -euo pipefail
cd "$(dirname "$0")/.."

echo "=== MCP security gateway ==="
uv run python -m gateways.mcp_gateway --downstream >/dev/null 2>&1 &
DOWNSTREAM=$!
uv run python -m gateways.mcp_gateway >/dev/null 2>&1 &
GATEWAY=$!
